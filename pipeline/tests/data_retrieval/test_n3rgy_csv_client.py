...
class TestN3rgyCSVClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = N3rgyCSVClient(
            source_dir=str(self.source_dir), output_dir=str(self.output_dir)
        )
        ...
        self.electricity_csv = (
            self.source_dir / "electricity_consumption_20250101_to_20250131.csv"
        )
    ...
    def test_extract_energy_type_from_filename(self):
        self.assertEqual(
            self.client._extract_energy_type_from_filename(
                "electricity_consumption_20250101"
            ),
            "electricity",
        )
        self.assertEqual(
        ...
        self.assertIsNone(
            self.client._extract_energy_type_from_filename(
                "energy_consumption_20250101"
            )
        )

    def test_extract_date_range_from_filename(self):
        self.assertEqual(
            self.client._extract_date_range_from_filename(
                "electricity_20250101_to_20250131"
            ),
    ...
    def test_transform_csv_to_json(self):
        consumption_path, cost_path = self.client.transform_csv_to_json(
            self.electricity_csv, "electricity"
        )
        ...
        with open(consumption_path, "r") as f:
            ...
            self.assertEqual(
                consumption_data["resource_name"], "electricity consumption"
            )
