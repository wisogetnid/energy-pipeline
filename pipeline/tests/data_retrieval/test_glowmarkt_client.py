...
def client(mock_token):
    return GlowmarktClient(
        token=mock_token, application_id="b0f1b774-a586-4f72-9edd-27ead8aa7a8d"
    )
...
class TestGlowmarktClientAuth:
    ...
    def test_authenticate_returns_token(self, mock_auth_response, auth_patch):
        client = GlowmarktClient(
            username="test", password="password", application_id="test-app-id"
        )
    ...
    def test_get_readings_auto_authenticates(
        self, mock_auth_response, sample_resource_id, mock_readings_response
    ):
        client = GlowmarktClient(
            username="test",
            password="password",
        ...
        with patch(
            "requests.post", return_value=mock_auth_response
        ) as mock_post, patch(
            "requests.get", return_value=mock_readings_response
        ) as mock_get:
...
class TestGlowmarktClientGetReadings:
    ...
    def test_get_readings_uses_correct_headers(
        self, client, mock_token, sample_resource_id, get_patch
    ):
        with get_patch as mock_get:
            ...
            assert (
                mock_get.call_args[1]["headers"]["applicationId"]
                == "b0f1b774-a586-4f72-9edd-27ead8aa7a8d"
            )

    def test_get_readings_returns_valid_data(
        self, client, sample_resource_id, mock_readings_response, get_patch
    ):
        with get_patch:
    ...
    def test_get_readings_with_date_range(
        ...
        sample_resource_id,
        sample_date_range,
        mock_empty_readings_response,
    ):
        with patch(
            "requests.get", return_value=mock_empty_readings_response
        ) as mock_get:
            client.get_readings(
                sample_resource_id,
    ...
    def test_get_readings_handles_datetime_objects(
        self,
        client,
        sample_resource_id,
    ...
    def test_get_readings_with_all_parameters(
        self, client, sample_resource_id, sample_date_range, mock_readings_response
    ):
...
class TestGlowmarktClientEntityAndResource:
    ...
    def test_get_virtual_entities(
        self, client, mock_token, mock_virtual_entities_response
    ):
        with patch(
            "requests.get", return_value=mock_virtual_entities_response
        ) as mock_get:
    ...
    def test_get_virtual_entity_resources(self, client, mock_ve_resources_response):
        ...
        with patch("requests.get", return_value=mock_ve_resources_response) as mock_get:
            ...
            mock_get.assert_called_once()
            assert (
                f"/virtualentity/{virtual_entity_id}/resources"
