...
class TestJsonlToParquetConverter:
    ...
    def large_jsonl_file(self, tmp_path):
        ...
        with open(file_path, "w") as f:
            for i in range(1000):
                ...
                entry["timestamp_iso"] = (
                    f"2025-01-01T{12 + (i // 2):02d}:{(i % 2) * 30:02d}:00"
                )
                entry["value"] = i * 0.1
    ...
    def test_convert_jsonl_to_parquet(self, converter, sample_jsonl_file):
        ...
        result_path = converter.convert_jsonl_to_parquet_file(
            sample_jsonl_file, output_file=output_file_path
        )
    ...
    def test_convert_combined_jsonl_to_parquet(self, converter, combined_jsonl_file):
        ...
        result_path = converter.convert_jsonl_to_parquet_file(
            combined_jsonl_file, output_file=output_file_path
        )
    ...
    def test_numeric_type_optimization_for_combined_data(
        self, converter, combined_jsonl_file
    ):
    ...
    def test_batch_conversion_of_multiple_files(
        self, converter, sample_jsonl_file, alternate_jsonl_file, combined_jsonl_file
    ):
    ...
    def test_preserves_all_columns_for_combined_data(
        self, converter, combined_jsonl_file
    ):
        result_path = converter.convert_jsonl_to_parquet_file(combined_jsonl_file)
    ...
    def test_convert_yearly_jsonl_to_parquet(self, converter, yearly_jsonl_file):
        ...
        result_path = converter.convert_jsonl_to_parquet_file(
            yearly_jsonl_file, output_file=output_file_path
        )
