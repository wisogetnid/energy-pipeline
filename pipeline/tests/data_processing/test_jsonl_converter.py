...
class TestEnergyDataConverter:
    ...
    def alternate_format_data(self):
        return {
            ...
            "data": [[1735689600, 10.5], [1735776000, 12.3]],
        }
    ...
    def test_convert_dictionary_to_jsonl(self, converter, gas_consumption_data):
        ...
        result_path = converter.convert_to_jsonl(
            gas_consumption_data, output_file=output_file_path
        )
    ...
    def test_convert_file_to_jsonl(self, converter, gas_consumption_file_path):
        ...
        result_path = converter.convert_to_jsonl(
            gas_consumption_file_path, output_file=output_file_path
        )
    ...
    def test_extract_resource_type(self, converter):
        assert (
            converter.extract_resource_type("electricity consumption") == "electricity"
        )
    ...
    def test_combine_consumption_and_cost(
        self, converter, gas_consumption_file_path, gas_cost_file_path
    ):
        combined_readings, metadata = converter.merge_consumption_and_cost_data(
            gas_consumption_file_path, gas_cost_file_path
        )
    ...
    def test_find_matching_resource_files(
        self, converter, tmp_path, electricity_consumption_data, electricity_cost_data
    ):
        test_data_dir = tmp_path / "test_matching"
        test_data_dir.mkdir()

        consumption_file = (
            test_data_dir / "electricity_consumption_20250101_to_20250131.json"
        )
        cost_file = test_data_dir / "electricity_cost_20250101_to_20250131.json"

        gas_consumption_file = (
            test_data_dir / "gas_consumption_20250101_to_20250131.json"
        )
    ...
    def test_combine_batch_resources(
        self, converter, tmp_path, electricity_consumption_data, electricity_cost_data
    ):
        ...
        assert (
            len(output_files) == 2
        ), f"Expected 2 output files, got {len(output_files)}"

        for output_file in output_files:
            assert Path(
                output_file
            ).exists(), f"Output file {output_file} does not exist"
            ...
            with open(output_file, "r") as f:
                ...
                ), f"Missing resource_type in {output_file}"
                assert data["resource_type"] in [
                    "electricity",
                    "gas",
                ], f"Invalid resource_type: {data['resource_type']}"
                assert (
                    "consumption_value" in data
                ), f"Missing consumption_value in {output_file}"
                assert "cost_value" in data, f"Missing cost_value in {output_file}"

    def test_batch_conversion_of_multiple_files(
        self, converter, gas_consumption_file_path, electricity_consumption_file_path
    ):
        output_file_paths = converter.batch_convert_to_jsonl(
            [str(gas_consumption_file_path), str(electricity_consumption_file_path)]
        )
    ...
    def test_combine_all_resources(
        self, converter, tmp_path, electricity_consumption_data, electricity_cost_data
    ):
