...
def mock_glowmarkt_responses():
    ...
    def mock_get(url, **kwargs):
        ...
        if url.endswith("/virtualentity"):
            mock_response.json.return_value = [{"veId": "ve-123", "name": "Test Home"}]
        elif "/virtualentity/ve-123/resources" in url:
            mock_response.json.return_value = {
                "resources": [
                    {
                        "resourceId": "res-elec-con",
                        "name": "Electricity consumption",
                        "baseUnit": "kWh",
                        "classifier": "electricity.consumption",
                    },
                    {
                        "resourceId": "res-elec-cost",
                        "name": "Electricity cost",
                        "baseUnit": "GBP",
                        "classifier": "electricity.cost",
                    },
                    {
                        "resourceId": "res-gas-con",
                        "name": "Gas consumption",
                        "baseUnit": "kWh",
                        "classifier": "gas.consumption",
                    },
                    {
                        "resourceId": "res-gas-cost",
                        "name": "Gas cost",
                        "baseUnit": "GBP",
                        "classifier": "gas.cost",
                    },
    ...
    with patch("requests.post", side_effect=mock_post), patch(
        "requests.get", side_effect=mock_get
    ):
        yield


def test_glowmarkt_happy_path(
    tmp_path, monkeypatch, mock_glowmarkt_responses, PatchedDateTime
):
    ...
    with patch("pipeline.ui.data_retrieval_ui.datetime", PatchedDateTime), patch(
        "pipeline.data_retrieval.batch_retrieval.datetime", PatchedDateTime
    ), patch(
        "pipeline.data_processing.jsonl_converter.datetime", PatchedDateTime
    ), patch(
        "pipeline.ui.data_retrieval_ui.get_latest_available_date"
    ) as mock_latest:
