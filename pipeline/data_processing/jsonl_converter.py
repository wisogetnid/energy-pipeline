...
class EnergyDataConverter:
    ...
    def convert_to_jsonl(self, input_file: Union[str, Path, Dict], output_file: Optional[Union[str, Path]] = None) -> str:
        ...
        start_date = input_data.get("start_date", input_data.get("query", {}).get("from", "unknown"))
        end_date = input_data.get("end_date", input_data.get("query", {}).get("to", "unknown"))
        ...
        logger.info(f"Converted {total_entries_written} readings to JSONL format at {output_jsonl_path}")
    ...
    def merge_consumption_and_cost_data(self, consumption_file: Union[str, Path], cost_file: Union[str, Path]) -> Tuple[Dict[int, Dict], Dict]:
        ...
        consumption_values_by_timestamp = {reading[0]: reading[1] for reading in consumption_readings}
        cost_values_by_timestamp = {reading[0]: reading[1] for reading in cost_readings}
        
        merged_readings_map = {}
        
        for timestamp in set(consumption_values_by_timestamp.keys()) | set(cost_values_by_timestamp.keys()):
    ...
    def combine_consumption_and_cost(self, consumption_file: Union[str, Path], cost_file: Union[str, Path], output_file: Optional[Union[str, Path]] = None) -> str:
        merged_readings_map, resource_metadata_details = self.merge_consumption_and_cost_data(consumption_file, cost_file)
        
        resource_type = resource_metadata_details["resource_type"]
        
        if output_file is None:
            consumption_path = Path(consumption_file)
            generated_filename = f"{resource_type}_combined_{consumption_path.name.split('_', 1)[1]}"
        ...
        with open(output_jsonl_path, 'w') as file_handle:
            for timestamp, reading in sorted(merged_readings_map.items()):
                combined_record = {
                    ...
                    "consumption_classifier": resource_metadata_details["consumption_classifier"],
        ...
        logger.info(f"Combined {total_entries_written} readings into JSONL format at {output_jsonl_path}")
        return str(output_jsonl_path)
    ...
    def find_matching_resource_files(self, directory: Union[str, Path]) -> List[Tuple[str, str]]:
        ...
        for file_path in all_json_files:
            ...
            if regex_match:
                extracted_date_range = f"{regex_match.group(1)}_to_{regex_match.group(2)}"
            
            if not extracted_date_range:
                fallback_base_name = lowercase_filename.replace("consumption", "").replace("cost", "").replace(".json", "")
        ...
        for key in consumption_files_by_unique_key:
            if key in cost_files_by_unique_key:
                matching_file_pairs_list.append((consumption_files_by_unique_key[key], cost_files_by_unique_key[key]))
        
        logger.info(f"Found {len(matching_file_pairs_list)} matching consumption-cost file pairs in {directory}")
    ...
    def combine_all_resources_into_single_file(self, directory: Union[str, Path] = "data/glowmarkt_api_raw", output_file: Optional[Union[str, Path]] = None, split_by_month: bool = True) -> Union[str, List[str]]:
        ...
        for consumption_file, cost_file in matching_file_pairs_list:
            readings_map, metadata = self.merge_consumption_and_cost_data(consumption_file, cost_file)
            ...
            for timestamp, reading in readings_map.items():
                ...
                combined_readings_map[timestamp][f"{res_type}_consumption"] = reading["consumption_value"]
                combined_readings_map[timestamp][f"{res_type}_cost"] = reading["cost_value"]
        ...
        for res_type, metadata in metadata_by_resource_type.items():
            consolidated_global_metadata[f"{res_type}_consumption_id"] = metadata["consumption_id"]
            consolidated_global_metadata[f"{res_type}_consumption_name"] = metadata["consumption_name"]
            consolidated_global_metadata[f"{res_type}_consumption_unit"] = metadata["consumption_unit"]
            consolidated_global_metadata[f"{res_type}_consumption_classifier"] = metadata["consumption_classifier"]
            consolidated_global_metadata[f"{res_type}_cost_id"] = metadata["cost_id"]
            consolidated_global_metadata[f"{res_type}_cost_name"] = metadata["cost_name"]
            consolidated_global_metadata[f"{res_type}_cost_unit"] = metadata["cost_unit"]
            consolidated_global_metadata[f"{res_type}_cost_classifier"] = metadata["cost_classifier"]
        
        if not split_by_month:
            if output_file is None:
                representative_consumption_file = matching_file_pairs_list[0][0]
                representative_data = self.load_json_from_file(representative_consumption_file)
                
                raw_from_date = representative_data.get("query", {}).get("from", representative_data.get("start_date", "unknown"))
                raw_to_date = representative_data.get("query", {}).get("to", representative_data.get("end_date", "unknown"))
                
                start_date_suffix = raw_from_date.split("T")[0].replace("-", "") if isinstance(raw_from_date, str) else "unknown"
                end_date_suffix = raw_to_date.split("T")[0].replace("-", "") if isinstance(raw_to_date, str) else "unknown"
                
                output_jsonl_filename = f"all_resources_{start_date_suffix}_to_{end_date_suffix}.jsonl"
            ...
            with open(output_jsonl_path, 'w') as file_handle:
                for timestamp, reading_data in sorted(combined_readings_map.items()):
                    fully_combined_record = {**consolidated_global_metadata, **reading_data}
                    file_handle.write(json.dumps(fully_combined_record) + '\n')
                    entries_written_count += 1
            
            logger.info(f"Combined {entries_written_count} readings across {len(metadata_by_resource_type)} resource types into JSONL format at {output_jsonl_path}")
            return str(output_jsonl_path)
        ...
        for timestamp, reading_data in combined_readings_map.items():
            try:
                if isinstance(timestamp, (int, float)):
                    ts_seconds = timestamp / 1000 if timestamp > 9999999999 else timestamp
                ...
                monthly_binned_readings[year_month_key].append((timestamp, reading_data))
            except Exception as timestamp_parse_error:
                logger.warning(f"Could not parse timestamp {timestamp}: {timestamp_parse_error}")
        ...
        for key, readings_list in monthly_binned_readings.items():
            ...
            if key == 'unknown':
                ...
                output_filename = f"all_resources_{range_start_suffix}_to_{range_end_suffix}.jsonl"
            ...
            with open(monthly_output_path, 'w') as file_handle:
                for timestamp, reading_entry in sorted(readings_list):
                    monthly_combined_record = {**month_specific_metadata, **reading_entry}
                    file_handle.write(json.dumps(monthly_combined_record) + '\n')
                    monthly_entries_written += 1
            
            logger.info(f"Created monthly file for {key} with {monthly_entries_written} readings at {monthly_output_path}")
            output_files_paths.append(str(monthly_output_path))
        
        logger.info(f"Split data into {len(output_files_paths)} monthly files in {self.output_dir}")
    ...
    def batch_combine_resource_files(self, directory: Union[str, Path]) -> List[str]:
        ...
        for consumption_file, cost_file in matching_file_pairs_list:
            output_file_path = self.combine_consumption_and_cost(consumption_file, cost_file)
