    ...
    def convert_jsonl_to_parquet_file(self, jsonl_file: Union[str, Path], output_file: Optional[Union[str, Path]] = None) -> Optional[str]:
        ...
        try:
            ...
            with open(jsonl_input_path, 'r') as file_handle:
                for line in file_handle:
                    try:
                        ...
                        logger.warning(f"Skipping invalid JSON line in {jsonl_input_path}: {decode_error}")
                        continue
            ...
            if output_file is None:
                parquet_output_path = self.output_dir / f"{jsonl_input_path.stem}.parquet"
            else:
                parquet_output_path = Path(output_file)
            
            parquet_output_path.parent.mkdir(parents=True, exist_ok=True)
            
            if 'timestamp' in energy_dataframe.columns and energy_dataframe['timestamp'].dtype == 'object':
                try:
                    energy_dataframe['timestamp'] = pd.to_numeric(energy_dataframe['timestamp'])
                except:
                    pass
            
            energy_dataframe.to_parquet(parquet_output_path, index=False)
            
            logger.info(f"Converted {jsonl_input_path} to Parquet format at {parquet_output_path}")
            return str(parquet_output_path)
            
        except Exception as conversion_error:
            logger.error(f"Error converting {jsonl_input_path} to Parquet: {conversion_error}")
            return None
