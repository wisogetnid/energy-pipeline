...
class N3rgyCSVClient:
    ...
    def process_all_files(self, extract_cost=True, combine_to_jsonl=False):
        ...
        for csv_file in csv_files_to_process:
            ...
            if not energy_type:
                print(
                    f"Warning: Could not determine energy type for {csv_file.name}, skipping..."
                )
                continue

            date_range_suffix = self._extract_date_range_from_filename(csv_file.stem)
            output_json_path = (
                self.output_dir / f"{energy_type}_consumption_{date_range_suffix}.json"
            )
    ...
    def transform_csv_to_json(
        ...
        source_csv_path,
        energy_type,
        destination_json_path=None,
        extract_cost_data=True,
    ):
        ...
        with open(source_csv_path, "r") as csv_file:
            ...
            for row in csv_reader:
                ...
                if (
                    not consumption_value_string
                    or consumption_value_string == "energyConsumption (kWh)"
                ):
                    continue

                try:
                    reading_datetime = datetime.datetime.strptime(
                        timestamp_string, "%Y-%m-%d %H:%M"
                    )
                    unix_timestamp = int(reading_datetime.timestamp())

                    if (
                        earliest_reading_timestamp is None
                        or reading_datetime < earliest_reading_timestamp
                    ):
                        earliest_reading_timestamp = reading_datetime
                    if (
                        latest_reading_timestamp is None
                        or reading_datetime > latest_reading_timestamp
                    ):
                        latest_reading_timestamp = reading_datetime

                    parsed_consumption_value = (
                        float(consumption_value_string)
                        if consumption_value_string
                        else 0
                    )
                    consumption_readings.append(
                        [unix_timestamp, parsed_consumption_value]
                    )

                    if (
                        extract_cost_data
                        and cost_value_string
                        and cost_value_string != "current £/day"
                        and cost_value_string.strip()
                    ):
                        parsed_cost_value_in_pence = float(cost_value_string) * 100
                        cost_readings.append(
                            [unix_timestamp, parsed_cost_value_in_pence]
                        )
        ...
        consumption_data_structure = {
            "resource_id": self.energy_resource_metadata[energy_type]["consumption"][
                "resource_id"
            ],
            "resource_name": self.energy_resource_metadata[energy_type]["consumption"][
                "resource_name"
            ],
            "resource_unit": self.energy_resource_metadata[energy_type]["consumption"][
                "resource_unit"
            ],
            "resource_classifier": self.energy_resource_metadata[energy_type][
                "consumption"
            ]["resource_classifier"],
            "start_date": (
                earliest_reading_timestamp.strftime("%Y-%m-%dT%H:%M:%S")
                if earliest_reading_timestamp
                else ""
            ),
            "end_date": (
                latest_reading_timestamp.strftime("%Y-%m-%dT%H:%M:%S")
        ...
        if destination_json_path is None:
            ...
            if earliest_reading_timestamp and latest_reading_timestamp:
                ...
                date_range_label = self._extract_date_range_from_filename(
                    source_file_path.stem
                )
            consumption_json_output_path = (
                self.output_dir / f"{energy_type}_consumption_{date_range_label}.json"
            )
        else:
        ...
        print(
            f"Created consumption JSON from {source_csv_path} at {consumption_json_output_path}"
        )
        print(
            f"Processed {len(consumption_readings)} readings from {earliest_reading_timestamp} to {latest_reading_timestamp}"
        )

        cost_json_output_path = None
        if extract_cost_data and cost_readings:
            cost_data_structure = {
                "resource_id": self.energy_resource_metadata[energy_type]["cost"][
                    "resource_id"
                ],
                "resource_name": self.energy_resource_metadata[energy_type]["cost"][
                    "resource_name"
                ],
                "resource_unit": self.energy_resource_metadata[energy_type]["cost"][
                    "resource_unit"
                ],
                "resource_classifier": self.energy_resource_metadata[energy_type][
                    "cost"
            ...
            cost_json_output_path = str(consumption_json_output_path).replace(
                "_consumption_", "_cost_"
            )
            ...
            print(
                f"Created cost JSON from {source_csv_path} at {cost_json_output_path}"
            )
            print(
                f"Processed {len(cost_readings)} cost readings from {earliest_reading_timestamp} to {latest_reading_timestamp}"
            )

        return consumption_json_output_path, cost_json_output_path

    def create_jsonl_from_json_files(
        self, source_json_paths, destination_jsonl_path=None
    ):
        date_components = []
        ...
        if destination_jsonl_path is None:
            destination_jsonl_path = (
                self.output_dir
                / f"all_resources_{earliest_processed_date}_to_{latest_processed_date}.jsonl"
            )

        jsonl_output_path = self._merge_json_files_into_jsonl(
            source_json_paths, destination_jsonl_path
        )
    ...
    def _merge_json_files_into_jsonl(self, source_json_paths, destination_jsonl_path):
        ...
        for resource_type_data in combined_energy_data.values():
            for category_data in resource_type_data.values():
                if category_data["start_date"]:
                    all_encountered_timestamps.append(
                        datetime.datetime.strptime(
                            category_data["start_date"], "%Y-%m-%dT%H:%M:%S"
                        )
                    )
                if category_data["end_date"]:
                    all_encountered_timestamps.append(
                        datetime.datetime.strptime(
                            category_data["end_date"], "%Y-%m-%dT%H:%M:%S"
                        )
                    )

        overall_earliest_timestamp = (
            min(all_encountered_timestamps) if all_encountered_timestamps else None
        )
        overall_latest_timestamp = (
            max(all_encountered_timestamps) if all_encountered_timestamps else None
        )
        ...
        consolidated_metadata = {
            "period": "PT30M",
            "from_date": (
                overall_earliest_timestamp.strftime("%Y-%m-%dT%H:%M:%S")
                if overall_earliest_timestamp
                else ""
            ),
            "to_date": (
                overall_latest_timestamp.strftime("%Y-%m-%dT%H:%M:%S")
        ...
        for energy_type, resource_type_data in combined_energy_data.items():
            for data_category, category_data in resource_type_data.items():
                ...
                consolidated_metadata[f"{field_prefix}_id"] = category_data[
                    "resource_id"
                ]
                consolidated_metadata[f"{field_prefix}_name"] = category_data[
                    "resource_name"
                ]
                consolidated_metadata[f"{field_prefix}_unit"] = category_data[
                    "resource_unit"
                ]
                consolidated_metadata[f"{field_prefix}_classifier"] = category_data[
                    "resource_classifier"
                ]
        ...
        for energy_type in ["electricity", "gas"]:
            for data_category in ["consumption", "cost"]:
                ...
                if f"{field_prefix}_id" not in consolidated_metadata:
                    consolidated_metadata[f"{field_prefix}_id"] = (
                        f"unknown-{energy_type}-{data_category}"
                    )
                    consolidated_metadata[f"{field_prefix}_name"] = (
                        f"{energy_type} {data_category}"
                    )
                    consolidated_metadata[f"{field_prefix}_unit"] = (
                        "kWh" if data_category == "consumption" else "pence"
                    )
                    consolidated_metadata[f"{field_prefix}_classifier"] = (
                        f"{energy_type}.{data_category}"
                        if data_category == "consumption"
                        else f"{energy_type}.consumption.cost"
                    )
        ...
        print(
            f"Contains {len(merged_readings_by_timestamp)} readings from {overall_earliest_timestamp} to {overall_latest_timestamp}"
        )
                    ...
                    if file_content.get("resource_id") == resource_id:
                        if start_date or end_date:
                            ...
                            for timestamp, value in file_content.get("readings", []):
                                reading_date_str = datetime.datetime.fromtimestamp(
                                    timestamp
                                ).strftime("%Y-%m-%d")
...
def main():
    command_line_argument_parser = argparse.ArgumentParser(
        description="Convert N3rgy CSV files to JSON and JSONL formats"
    )
    ...
    command_line_argument_parser.add_argument(
        "--output-dir", default="./processed", help="Directory to save output files"
    )
    command_line_argument_parser.add_argument(
        "--no-cost", action="store_true", help="Skip extracting cost data"
    )
    command_line_argument_parser.add_argument(
        "--combine",
        action="store_true",
        help="Combine all data into a single JSONL file",
    )

    parsed_arguments = command_line_argument_parser.parse_args()

    n3rgy_client = N3rgyCSVClient(
        parsed_arguments.source_dir, parsed_arguments.output_dir
    )
