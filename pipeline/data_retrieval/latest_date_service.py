...
def get_latest_available_date(data_dir: str, resource_names: List[str], default_date: datetime = datetime(2024, 1, 1)) -> datetime:
    ...
    for resource in resource_names:
        ...
        filename_pattern = re.compile(rf"^{re.escape(safe_resource_name)}_(\d{{8}})_to_(\d{{8}})\.json$")
    ...
    furthest_behind_resource = min(resource_to_latest_end_date, key=resource_to_latest_end_date.get)
