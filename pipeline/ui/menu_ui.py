...
class MenuUI(BaseUI):
    ...
    def retrieve_data_menu(self):
        ...
        if choice == 1:
            ...
            if self.retrieval_ui.client and self.retrieval_ui.client_type == "n3rgy":
                ...
                json_files = self.retrieval_ui.client.process_all_files(
                    extract_cost=True, combine_to_jsonl=True
                )
                ...
                print(
                    "Error: process_all_files is not supported for the current client type or client is not initialized."
                )
    ...
    def visualisation_menu(self):
        ...
        if choice == 1:
            ...
            if result:
                ...
                print(
                    "Check the data/visualisations/monthly_summary directory for the charts."
                )
            else:
                print(
                ...
                print(
                    "You may need to run the 'Convert to Yearly JSONL files' option in the Convert Data menu first."
                )
            self.wait_for_user()
        elif choice == 2:
            from pipeline.ui.visualization_ui import VisualizationUI
            ...
            if result:
                ...
                print(
                    "Check the data/visualisations/theoretical_comparison directory for the chart."
                )
            else:
                print(
    ...
    def display_api_token(self):
        ...
        if token:
            ...
            try:
                if not self.retrieval_ui.client or not isinstance(
                    self.retrieval_ui.client, GlowmarktClient
                ):
                    self.retrieval_ui.client_type = "glowmarkt"
                    self.retrieval_ui.setup_glowmarkt_client(username, password)

                if self.retrieval_ui.client and isinstance(
                    self.retrieval_ui.client, GlowmarktClient
                ):
