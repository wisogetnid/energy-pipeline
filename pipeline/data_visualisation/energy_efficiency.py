...
def find_resource_data(resource_type="all"):
    ...
    file_pattern = (
        f"{resource_type.lower()}_consumption_*.parquet"
        if resource_type != "all"
        else "*_consumption_*.parquet"
    )
...
def generate_consumption_patterns(df, resource_type, consumption_unit, output_dir):
    ...
    pivot_data = df.pivot_table(
        index="day_name", columns="hour", values="value", aggfunc="mean"
    )
    ...
    heatmap = sns.heatmap(
        ...
        annot=False,
        fmt=".2f",
        cbar_kws={"label": consumption_unit},
    )
...
def generate_weekly_comparison(df, resource_type, consumption_unit, output_dir):
    weekly_consumption = (
        df.groupby(["week", "date"]).agg({"value": ["sum", "first"]}).reset_index()
    )

    weekly_consumption.columns = ["week", "date", "total_consumption", "first_value"]

    weekly_summary = (
        weekly_consumption.groupby("week")
        .agg({"total_consumption": "mean", "date": "min"})
        .reset_index()
    )
    ...
    bars = plt.bar(
        range(len(weekly_summary)), weekly_summary["total_consumption"], color="green"
    )
...
def generate_weekday_weekend_pattern(df, resource_type, consumption_unit, output_dir):
    ...
    hourly_by_day_type = (
        df.groupby(["hour", "is_weekend"]).agg({"value": "mean"}).reset_index()
    )
...
def generate_all_visualizations(file_path, output_dir):
    ...
    try:
        ...
        pattern_file = generate_consumption_patterns(
            df, resource_type, unit, output_dir
        )
