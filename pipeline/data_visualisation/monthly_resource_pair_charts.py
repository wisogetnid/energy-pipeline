...
def generate_visualizations(cost_file_path, consumption_file_path, output_dir=None):
    try:
        ...
        detected_resource_type = cost_df['resource_name'].iloc[0].split()[0].capitalize()
        ...
        cost_unit_label = cost_df['units'].iloc[0] if 'units' in cost_df.columns else 'pence'
        consumption_unit_label = consumption_df['units'].iloc[0] if 'units' in consumption_df.columns else 'kWh'
        
        sns.set_style("whitegrid")
        plt.rcParams.update({'font.size': 12})
        
        fig, (cost_axis, consumption_axis) = plt.subplots(2, 1, figsize=(15, 10), sharex=True)
        
        cost_axis.plot(merged_metrics_df['timestamp'], merged_metrics_df['value_cost'], color='red', linewidth=1.5)
        cost_axis.set_ylabel(f'Cost ({cost_unit_label})')
        cost_axis.set_title(f'{detected_resource_type} Cost Over Time')
        cost_axis.grid(True)
        
        consumption_axis.plot(merged_metrics_df['timestamp'], merged_metrics_df['value_consumption'], color='blue', linewidth=1.5)
        ...
        plt.tight_layout()
        plt.savefig(vis_dir / f'{detected_resource_type.lower()}_time_series.png', dpi=300)
        plt.close()
        
        hourly_averaged_data = merged_metrics_df.groupby('hour').agg({
            'value_cost': 'mean',
        ...
        cost_secondary_axis.plot(hourly_averaged_data['hour'], hourly_averaged_data['value_cost'], color='red', marker='o')
        ...
        consumption_secondary_axis.set_ylabel(f'Consumption ({consumption_unit_label})', color='blue')
        consumption_secondary_axis.plot(hourly_averaged_data['hour'], hourly_averaged_data['value_consumption'], color='blue', marker='s')
        ...
        plt.tight_layout()
        plt.savefig(vis_dir / f'{detected_resource_type.lower()}_hourly_patterns.png', dpi=300)
        plt.close()
        ...
        sns.heatmap(daily_hourly_consumption, cmap='YlGnBu', annot=False, fmt=".2f", cbar_kws={'label': consumption_unit_label})
        plt.title(f'{detected_resource_type} Consumption by Day and Hour')
        plt.xlabel('Hour of Day')
        plt.ylabel('Day of Month')
        plt.tight_layout()
        plt.savefig(vis_dir / f'{detected_resource_type.lower()}_consumption_heatmap.png', dpi=300)
        plt.close()
        
        daily_totals = merged_metrics_df.groupby('date').agg({
            'value_cost': 'sum',
        ...
        ax2 = ax1.twinx()
        ax2.set_ylabel(f'Total Daily Consumption ({consumption_unit_label})', color='blue')
        ...
        plt.tight_layout()
        plt.savefig(vis_dir / f'{detected_resource_type.lower()}_daily_totals.png', dpi=300)
        plt.close()
